...
resource "kubernetes_service_account" "vault_raft_operator" {
  metadata {
    ...
    annotations = {
      "azure.workload.identity/client-id" = azurerm_user_assigned_identity.vault_raft_operator.client_id
    }
    labels = {
...
resource "kubernetes_cron_job_v1" "vault_raft_backup" {
        ...
        template {
          ...
          spec {
            ...
            container {
              ...
              # NO envFrom - Workload Identity uses managed identity, no secrets needed
            }
          }
        }
      }
  ...
  depends_on = [
    ...
    azurerm_federated_identity_credential.vault_raft_operator,
